...
import { Picker, Footer, FooterTab, Drawer, Header, Container, Content, Card, Left, Body, Right, Title, Subtitle, Icon, H2, Item, Input, CardItem, ListItem, Button } from 'native-base';
...
class Home extends Component{
  ...
  componentDidMount(){
    ...
    var url= `${baseUrl}/data_barang?status=0&outlet=${this.state.outlet}`;
    console.log(url);
    axios.get(`${baseUrl}/data_barang?status=0&outlet=${this.state.outlet}&role=${this.props.dataGeneral.role}`)
    .then((res)=>{
  ...
  onCari(){
    ...
    var url = `${baseUrl}/cari_barang/?dept=${this.state.selected}&status=0&id=${this.state.textCari}&outlet=${this.state.outlet}`;
    console.log(url);
    axios.get(url)
    .then((res)=>{
      //console.log(res.data);
  ...
  renderItem(item){
    ...
    return(
      <Card transparent style={{ elevation: 0, borderBottomWidth: 1}}>
        <Row style={{ height: 90}}>
          <Col style={{ justifyContent: 'center', paddingLeft: 10}} onPress={()=>
            {
            ...
            <Text style={{ fontSize: 14, color: 'grey', fontFamily: 'Raleway-Regular'}}>Departement : {item.item.dept}</Text>
          </Col>
          <Col style={{ width:70, height: 70,  padding: 5, alignSelf: 'center'}}>
            <View style={{ flex: 1, backgroundColor: '#f1f8e9', borderRadius: 5, justifyContent: 'center', alignItems: 'center'}}>
  ...
  onValueChange = async(value) => {
    ...
    axios.get(`${baseUrl}/data_barang?dept=${this.state.selected}&status=0&outlet=${this.state.outlet}&role=${this.props.dataGeneral.role}`)
    .then((res)=>{
      //console.log(res.data);
    ...
    return(
      ...
      <Drawer  openDrawerOffset={0.4} tapToClose={true} ref={(ref) => { this.drawer = ref; }} content={<Sidebar closeDrawer={this.closeDrawer.bind(this)} navigator={this.props.navigation} />} onClose={() => this.closeDrawer()} >
       ...
       <StatusBar barStyle = "default" hidden = {false} backgroundColor = "white" />
          <Header style={{ backgroundColor: '#7cb342'}}>
            <Left style={{ flex: 1}}>
            ...
            <Body style= {{ flex: 5}}>
              <Text style={styles.headerStyle}>{this.state.judul + ' - List SO' || ''}</Text>
            </Body>
            ...
            <Grid>
              <Row style={{ height: 120 }}>
                <Col>
                <View style={{  backgroundColor: '#7cb342', height: 120, margin: 0, paddingLeft: 16, paddingRight: 16, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 }}>
                  <Row style={{ height: 50}}>
            ...
            <Footer>
              <FooterTab>
                ...
                <Button style={{ backgroundColor: '#03a9f4'}} onPress={()=> {
                    //this.props.navigation.pop();
